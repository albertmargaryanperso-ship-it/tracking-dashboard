          ...
          <div className="flex gap-1.5 mt-1">
            {(['normal', 'urgent'] as TodoPriority[]).map(p => (
              <button key={p} onClick={() => setPriority(p)}
                className={cn('flex-1 px-3 py-2 rounded-lg text-[11px] font-semibold border transition-all',
                  priority === p ? p === 'urgent' ? 'border-rose-500 bg-rose-500/10 text-rose-300' : 'border-zinc-700 bg-zinc-800 text-zinc-200' : 'border-zinc-800 text-zinc-500')}>
